...
class GUI:
    ...
    def _data_procesor_init_(self):
        filter_enable = self.gui_filter_chkbox.get_active()
        roughness = self.gui_filter_rough_adj.get_value()
        frequency = int(self.gui_freq_txtbox.get_text())
        self.data_processor = DataProcessor(output_graph = self.graph, 
                                            ...
                                            do_filtering = filter_enable,
                                            filter_freq = frequency,
                                            filter_roughness = roughness)
    ...
    def on_filter_toggled(self, *args):        
        ...
        if self.gui_filter_chkbox.get_active():
            self.data_processor.enable_filter()
        else:
            self.data_processor.disable_filter()
